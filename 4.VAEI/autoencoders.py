...
#            tf.train.AdamOptimizer(learning_rate=self.learning_rate).minimize(self.cost)
    ...
    def impute(self, X_corrupt, max_iter = 10):
        ...
        x_miss_val = X_corrupt[NanRowIndex[0],:]                      # All incomplete tuples
        
        # initialise missing values with arbitrary value "0"
        NanIndex = np.where(np.isnan(x_miss_val))
        x_miss_val[NanIndex] = 0
        
        MissVal = np.zeros([max_iter,len(NanIndex[0])], dtype=np.float32)    # Each missing value will get "max_iter" values.
    ...
    def train(self, XData, training_epochs=100, display_step=10):
        ...
        NanRowIndex = np.where(np.isnan(np.sum(XData,axis=1)))   # The array insist of index of incomplete data
        n_samples = np.size(XData, 0) - NanRowIndex[0].shape[0]  # Number of rows with complete entries in XData
        n_samples = np.size(XData, 0)
        
        losshistory = []
        losshistory_epoch = []
        for epoch in range(training_epochs):
            avg_cost = 0
            total_batch = int(n_samples / self.batch_size)   # batch iterations number
            # Loop over all batches
            for i in range(total_batch):
                batch_xs = next_batch(XData, self.batch_size, MissingVals=True)    # This returns complete records only:
                # Fit training using batch data
                cost = self.partial_fit(batch_xs)     # Training model and return cost of mini-batch.
