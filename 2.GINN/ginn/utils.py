...
def degrade_dataset(X, missingness, rand, rank, v):
    """
    Inputs:
        dataset to corrupt
    ...
    """
    Dimension = len(X[0])
    Top_K = int((0.5 + 0.5 * missingness) * Dimension)
    Top_feature = rank[:Top_K]
    Button_feature = rank[Top_K:]


    # X_1d = X.flatten()
    cX = X
    n = len(X)
    mask = np.ones([n, Dimension])

    for dim in range(Dimension):
        if dim in Top_feature:
